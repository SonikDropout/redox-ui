...
const { name: progName } = require('../../package.json');

module.exports = {
  logsDir: path.join(homedir, progName, 'logs'),
  baseName: progName.split('-').slice(0, -1).map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(''),
  fileName: `${this.baseName}.tsv`,
  async createFile(headers) {
    if (this.ws) this.clear();
    await this._createDir();
    this.fileName = `${this.baseName}_${await this._getFileID()}.tsv`;
